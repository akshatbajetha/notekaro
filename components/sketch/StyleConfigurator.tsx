...
import {
  ...
  textAlignIcons,
  textAlignLabels,
} from "@/config/canvasTypeMappings";
...
export function StyleConfigurator({
  ...
  const fillStyleOptions: FillStyle[] = [
    "hachure",
    "cross-hatch",
    "dashed",
    "dots",
...
const TextAlignSelector = ({
  ...
  return (
    ...
    >
      <Input
        type="radio"
        checked={textAlign === textAlignProp}
        onChange={() => onClick?.()}
        name="textAlign"
...
const FontFamilySelector = ({
  ...
  return (
    ...
    >
      <Input
        type="radio"
        checked={fontFamily === fontFamilyProp}
        onChange={() => onClick?.()}
        name="fontFamily"
        className="opacity-0 absolute pointer-events-none"
      />
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="w-4 rounded-[10px] bg-[#1e1e1e] dark:bg-[#d3d3d3]"
      />
    </label>
  );
};

const FillStyleSelector = ({
  fillStyle,
  fillStyleProp,
  onClick,
}: {
  fillStyle: FillStyle;
  fillStyleProp: FillStyle;
  onClick?: () => void;
}) => {
  return (
    <label
    ...
    >
      <Input
        type="radio"
        checked={fillStyle === fillStyleProp}
        onChange={() => onClick?.()}
        name="strokeWidth"
        className="opacity-0 absolute pointer-events-none"
      />
...
const RoughStyleSelector = ({
  roughStyle,
  roughStyleProp,
  onClick,
}: {
  roughStyle: RoughStyle;
  roughStyleProp: RoughStyle;
  onClick?: () => void;
}) => {
  return (
    <label
    ...
    >
      <Input
        type="radio"
        checked={roughStyle === roughStyleProp}
        onChange={() => onClick?.()}
        name="roughStyle"
        className="opacity-0 absolute pointer-events-none"
      />
